defmodule SelectoDBMSSQL.WriteCompiler do
  @moduledoc false
  ...
  def preview(%Batch{commands: commands}, opts) do
    with {:ok, statements} <- map_commands(commands, &compile(&1, opts)) do
      {:ok, %Preview{statements: statements, metadata: %{dialect: :mssql, atomic?: true}}}
    end
  end

  def preview(%Command{} = command, opts) do
    with {:ok, statement} <- compile(command, opts) do
      {:ok, %Preview{statements: [statement], metadata: %{dialect: :mssql}}}
    end
  end
  ...
  defp compile_insert(command, opts) do
    ...
    with {:ok, assignments} <- compile_assignments(command.assignments, opts),
         ...
         {:ok, returning} <- compile_returning(command.returning, :inserted),
  ...
  defp compile_upsert(command, opts) do
    ...
    with {:ok, assignments} <- compile_assignments(command.assignments, opts),
         ...
         {:ok, conflict_fields} <- compile_conflict_target(command.metadata),
      ...
      source = Enum.zip_with(values, columns, &"#{&1} AS #{&2}") |> Enum.join(", ")
      on = Enum.map_join(conflict_fields, " AND ", &"target.#{&1} = source.#{&1}")
      guard = if guards.text, do: " AND (#{guards.text})", else: ""

      matched =
        case update_assignments do
          [] ->
            [field | _] = conflict_fields
            " WHEN MATCHED#{guard} THEN UPDATE SET target.#{field} = target.#{field}"

          fields ->
            set = Enum.map_join(fields, ", ", &"target.#{&1.column} = source.#{&1.column}")
            " WHEN MATCHED#{guard} THEN UPDATE SET #{set}"
        end

      {:ok,
       %{
         text:
           "MERGE INTO #{quote_relation(command.relation)} WITH (HOLDLOCK) AS target " <>
             "USING (SELECT #{source}) AS source ON #{on}" <>
             matched <>
             " WHEN NOT MATCHED#{guard} THEN INSERT (#{Enum.join(columns, ", ")}) " <>
             "VALUES (#{Enum.map_join(columns, ", ", &("source." <> &1))})" <>
             returning <> ";",
         params: params ++ guards.params
       }}
    end
  ...
  defp compile_update(command, opts) do
    ...
    with {:ok, assignments} <- compile_assignments(command.assignments, opts),
      ...
      {:ok,
       %{
         text:
           "UPDATE #{quote_relation(command.relation)} SET #{set}" <>
             returning <> " WHERE #{predicate.text}" <> guard_text,
  ...
  defp compile_delete(command, opts) do
    with {:ok, predicate} <- compile_predicate(command.predicate, opts, 0),
         {:ok, returning} <- compile_returning(command.returning, :deleted) do
      {:ok,
       %{
         text:
           "DELETE FROM #{quote_relation(command.relation)}" <>
             returning <> " WHERE #{predicate.text}",
  ...
  def compile_predicate(predicate, _opts, _offset) do
    {:error,
     Error.new(:invalid_predicate, "unsupported portable SQL Server predicate",
  ...
  defp compile_returning(:none, _row_alias), do: {:ok, ""}
  defp compile_returning(:all, row_alias), do: {:ok, " OUTPUT #{alias_name(row_alias)}.*"}

  defp compile_returning(fields, row_alias) when is_list(fields),
    do:
      {:ok,
       " OUTPUT " <>
         Enum.map_join(fields, ", ", &(alias_name(row_alias) <> "." <> quote_identifier(&1)))}

  defp compile_returning(value, _row_alias),
    do:
      {:error,
    ...
    case Map.get(metadata, :conflict_target) do
      fields when is_list(fields) and fields != [] ->
        if Enum.all?(fields, &valid_ref?/1),
          do: {:ok, Enum.map(fields, &quote_identifier/1)},
  ...
  defp alias_name(:inserted), do: "INSERTED"
  defp alias_name(:deleted), do: "DELETED"
